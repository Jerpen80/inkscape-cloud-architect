...
### Requirement: Column width accounts for EC2 card content
Subnet column width SHALL grow to fit EC2 instance card content (icon + name/type at configured scale).

#### Scenario: Wide instance name
- **WHEN** an EC2 instance name or type at the configured font_size and icon_scale is wider than the subnet label
- **THEN** the column width SHALL expand to fit the widest EC2 card
